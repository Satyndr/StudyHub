...
import { BuyCourse, BuyCourseFree } from '../services/operations/studentFeaturesAPI'
...
const CourseDetails = () => {
    ...
    const handelPayment = () => {
        if(token){
            // BuyCourse(token,[courseId],user,navigate,dispatch);  //real function
            BuyCourseFree(token, [courseId], user, navigate, dispatch);
        }
        else{
            navigate('/login');
        }
